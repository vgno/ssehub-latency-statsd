// Original source: https://github.com/aslakhellesoy/eventsource
...
function EventSource(url, eventSourceInitDict) {
  ...
  if (eventSourceInitDict && eventSourceInitDict.headers && isPlainObject(eventSourceInitDict.headers) && eventSourceInitDict.headers['Last-Event-ID']) {
    id = eventSourceInitDict.headers['Last-Event-ID'];
  ...
  function connect() {
    ...
    if (id) options.headers['Last-Event-ID'] = id;
    ...
    if (lineLength === 0) {
      if (data.length > 0) {
        ...
        _emit('message', new MessageEvent(type, {
          type: type,
          msg: data.slice(0, -1), // remove trailing newline
          id: id,
          origin: original(url)
        }));
        data = '';
